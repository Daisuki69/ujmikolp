...
public final class CacheInterceptor implements Interceptor {
    ...
    public static final class Companion {
        ...
        public final Headers combine(Headers headers, Headers headers2) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if ((!"Warning".equalsIgnoreCase(strName) || !z.w(strValue, 2, IcyHeaders.REQUEST_HEADER_ENABLE_METADATA_VALUE, false)) && (isContentSpecificHeader(strName) || !isEndToEnd(strName) || headers2.get(strName) == null)) {
                    builder.addLenient$okhttp(strName, strValue);
                }
            }
            int size2 = headers2.size();
            for (int i4 = 0; i4 < size2; i4++) {
                String strName2 = headers2.name(i4);
                if (!isContentSpecificHeader(strName2) && isEndToEnd(strName2)) {
                    builder.addLenient$okhttp(strName2, headers2.value(i4));
                }
            }
    ...
    private final Response cacheWritingResponse(CacheRequest cacheRequest, Response response) throws IOException {
        ...
        CacheInterceptor$cacheWritingResponse$cacheWritingSource$1 cacheInterceptor$cacheWritingResponse$cacheWritingSource$1 = new CacheInterceptor$cacheWritingResponse$cacheWritingSource$1(responseBodyBody.source(), cacheRequest, Okio.buffer(sinkBody));
        return response.newBuilder().body(new RealResponseBody(Response.header$default(response, RtspHeaders.CONTENT_TYPE, null, 2, null), response.body().contentLength(), Okio.buffer(cacheInterceptor$cacheWritingResponse$cacheWritingSource$1))).build();
    }
