...
public class DividedDateTimeField extends DecoratedDateTimeField {
    ...
    public int getDifference(long j, long j6) {
        return getWrappedField().getDifference(j, j6) / this.iDivisor;
    }

    @Override // org.joda.time.field.BaseDateTimeField, org.joda.time.DateTimeField
    public long getDifferenceAsLong(long j, long j6) {
        return getWrappedField().getDifferenceAsLong(j, j6) / ((long) this.iDivisor);
    }
    ...
    public DividedDateTimeField(DateTimeField dateTimeField, DurationField durationField, DateTimeFieldType dateTimeFieldType, int i) {
        ...
        int i4 = minimumValue >= 0 ? minimumValue / i : ((minimumValue + 1) / i) - 1;
        int maximumValue = dateTimeField.getMaximumValue();
        int i6 = maximumValue >= 0 ? maximumValue / i : ((maximumValue + 1) / i) - 1;
        this.iMin = i4;
        this.iMax = i6;
    }
    ...
    public long add(long j, long j6) {
        return getWrappedField().add(j, j6 * ((long) this.iDivisor));
    }
