...
public class SystemJobService extends JobService implements ExecutionListener {
    ...
    private final StartStopTokens mStartStopTokens = d.d(false);
    ...
    private static void assertMainThread(String str) {
        if (Looper.getMainLooper().getThread() != Thread.currentThread()) {
            throw new IllegalStateException(s.j("Cannot invoke ", str, " on a background thread"));
        }
    }
    ...
    public void onExecuted(@NonNull WorkGenerationalId workGenerationalId, boolean z4) {
        ...
        if (jobParametersRemove != null) {
            jobFinished(jobParametersRemove, z4);
        }
    }
