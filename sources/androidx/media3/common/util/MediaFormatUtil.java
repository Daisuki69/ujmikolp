...
public final class MediaFormatUtil {
    ...
    public static Format createFormatFromMediaFormat(MediaFormat mediaFormat) {
        ...
        while (true) {
            ...
            if (byteBuffer == null) {
                pcmEncoding.setInitializationData(S.i(i4, objArrCopyOf));
    ...
    public static void setCsdBuffers(MediaFormat mediaFormat, List<byte[]> list) {
        for (int i = 0; i < list.size(); i++) {
            mediaFormat.setByteBuffer(s.f(i, "csd-"), ByteBuffer.wrap(list.get(i)));
        }
    }

    @Nullable
    private static ColorInfo getColorInfo(MediaFormat mediaFormat, boolean z4) {
        if (Util.SDK_INT < 24) {
            return null;
        }
