...
final class MpeghUtil {
    ...
    public static class Mpegh3daConfig {
        ...
        private Mpegh3daConfig(int i, int i4, int i6, @Nullable byte[] bArr) {
            this.profileLevelIndication = i;
            this.samplingFrequency = i4;
            this.standardFrameLength = i6;
        ...
        if (parsableBitArray.readBit()) {
            ...
            for (int i = 0; i < escapedIntValue; i++) {
                ...
                if (escapedIntValue2 == 7) {
                    ...
                    for (int i4 = 0; i4 < bits5; i4++) {
                        bArr2[i4] = (byte) parsableBitArray.readBits(8);
                    }
                    bArr = bArr2;
                } else {
    ...
    private static int readEscapedIntValue(ParsableBitArray parsableBitArray, int i, int i4, int i6) {
        Assertions.checkArgument(Math.max(Math.max(i, i4), i6) <= 31);
        int i10 = (1 << i) - 1;
        int i11 = (1 << i4) - 1;
        f.h(f.h(i10, i11), 1 << i6);
        ...
        if (bits == i10) {
            ...
            if (bits2 == i11) {
                ...
                return parsableBitArray.readBits(i6) + bits;
            }
        }
        return bits;
    }

    private static long readEscapedLongValue(ParsableBitArray parsableBitArray, int i, int i4, int i6) {
        Assertions.checkArgument(Math.max(Math.max(i, i4), i6) <= 63);
        long j = (1 << i) - 1;
        long j6 = (1 << i4) - 1;
        i.h(i.h(j, j6), 1 << i6);
        if (parsableBitArray.bitsLeft() < i) {
        ...
        if (bitsToLong == j) {
            ...
            long bitsToLong2 = parsableBitArray.readBitsToLong(i4);
            bitsToLong += bitsToLong2;
            if (bitsToLong2 == j6) {
                if (parsableBitArray.bitsLeft() < i6) {
                    return -1L;
                }
                return parsableBitArray.readBitsToLong(i6) + bitsToLong;
            }
        }
        return bitsToLong;
    }

    private static void skipMpegh3daDecoderConfig(ParsableBitArray parsableBitArray, int i, int i4) {
        int bits;
    ...
    private static void skipMpegh3daFlexibleSpeakerConfig(ParsableBitArray parsableBitArray, int i) {
        ...
        while (i11 < i) {
            if (parsableBitArray.readBit()) {
                ...
                if (parsableBitArray.readBits(2) == 3 && parsableBitArray.readBits(i6) * i4 != 0) {
                    parsableBitArray.skipBit();
                }
                bits = parsableBitArray.readBits(i10) * i4;
                if (bits != 0 && bits != 180) {
                    parsableBitArray.skipBit();
                }
                parsableBitArray.skipBit();
