...
final class PsBinarySearchSeeker extends BinarySearchSeeker {
    ...
    public static final class PsScrSeeker implements BinarySearchSeeker.TimestampSeeker {
        ...
        private BinarySearchSeeker.TimestampSearchResult searchForScrValueInBuffer(ParsableByteArray parsableByteArray, long j, long j6) {
            int position = -1;
            int position2 = -1;
                ...
                if (PsBinarySearchSeeker.peekIntAtPosition(parsableByteArray.getData(), parsableByteArray.getPosition()) != 442) {
                    ...
                    if (scrValueFromPack != C.TIME_UNSET) {
                        ...
                        if (jAdjustTsTimestamp > j) {
                            return j7 == C.TIME_UNSET ? BinarySearchSeeker.TimestampSearchResult.overestimatedResult(jAdjustTsTimestamp, j6) : BinarySearchSeeker.TimestampSearchResult.targetFoundResult(j6 + ((long) position2));
                        }
                        if (100000 + jAdjustTsTimestamp > j) {
                            return BinarySearchSeeker.TimestampSearchResult.targetFoundResult(j6 + ((long) parsableByteArray.getPosition()));
                        }
                        position2 = parsableByteArray.getPosition();
                        j7 = jAdjustTsTimestamp;
                    }
                    skipToEndOfCurrentPack(parsableByteArray);
                    position = parsableByteArray.getPosition();
                }
            }
            return j7 != C.TIME_UNSET ? BinarySearchSeeker.TimestampSearchResult.underestimatedResult(j7, j6 + ((long) position)) : BinarySearchSeeker.TimestampSearchResult.NO_TIMESTAMP_IN_RANGE_RESULT;
        }
    ...
    public PsBinarySearchSeeker(TimestampAdjuster timestampAdjuster, long j, long j6) {
        super(new BinarySearchSeeker.DefaultSeekTimestampConverter(), new PsScrSeeker(timestampAdjuster), j, 0L, j + 1, 0L, j6, 188L, 1000);
    }
