...
public final class DashMediaSource extends BaseMediaSource {
    ...
    public static final class DashTimeline extends Timeline {
        ...
        public DashTimeline(long j, long j6, long j7, int i, long j9, long j10, long j11, DashManifest dashManifest, MediaItem mediaItem, @Nullable MediaItem.LiveConfiguration liveConfiguration) {
        ...
        private long getAdjustedWindowDefaultStartPositionUs(long j) {
            ...
            int i = 0;
            while (i < this.manifest.getPeriodCount() - 1 && j7 >= periodDurationUs) {
                j7 -= periodDurationUs;
                i++;
            ...
            return (adaptationSetIndex == -1 || (index = period.adaptationSets.get(adaptationSetIndex).representations.get(0).getIndex()) == null || index.getSegmentCount(periodDurationUs) == 0) ? j6 : (index.getTimeUs(index.getSegmentNum(j7, periodDurationUs)) + j6) - j7;
        }
        ...
        public Timeline.Period getPeriod(int i, Timeline.Period period, boolean z4) {
            ...
            return period.set(z4 ? this.manifest.getPeriod(i).id : null, z4 ? Integer.valueOf(this.firstPeriodId + i) : null, 0, this.manifest.getPeriodDurationUs(i), Util.msToUs(this.manifest.getPeriod(i).startMs - this.manifest.getPeriod(0).startMs) - this.offsetInFirstPeriodUs);
        }
    ...
    public static final class Factory implements MediaSourceFactory {
        ...
        public Factory experimentalParseSubtitlesDuringExtraction(boolean z4) {
            this.chunkSourceFactory.experimentalParseSubtitlesDuringExtraction(z4);
            return this;
        }
    ...
    public static final class Iso8601Parser implements ParsingLoadable.Parser<Long> {
        ...
        public Long parse(Uri uri, InputStream inputStream) throws IOException {
            String line = new BufferedReader(new InputStreamReader(inputStream, e.c)).readLine();
            try {
                ...
                if (!"Z".equals(matcher.group(2))) {
                    ...
                    time -= (((j6 * 60) + (TextUtils.isEmpty(strGroup2) ? 0L : Long.parseLong(strGroup2))) * 60000) * j;
                }
    ...
    public final class ManifestCallback implements Loader.Callback<ParsingLoadable<DashManifest>> {
        ...
        public void onLoadCanceled(ParsingLoadable<DashManifest> parsingLoadable, long j, long j6, boolean z4) {
            DashMediaSource.this.onLoadCanceled(parsingLoadable, j, j6);
        }

        @Override // androidx.media3.exoplayer.upstream.Loader.Callback
        public void onLoadCompleted(ParsingLoadable<DashManifest> parsingLoadable, long j, long j6) {
            DashMediaSource.this.onManifestLoadCompleted(parsingLoadable, j, j6);
        }

        @Override // androidx.media3.exoplayer.upstream.Loader.Callback
        public Loader.LoadErrorAction onLoadError(ParsingLoadable<DashManifest> parsingLoadable, long j, long j6, IOException iOException, int i) {
            return DashMediaSource.this.onManifestLoadError(parsingLoadable, j, j6, iOException, i);
        }
    }
    ...
    public final class UtcTimestampCallback implements Loader.Callback<ParsingLoadable<Long>> {
        ...
        @Override // androidx.media3.exoplayer.upstream.Loader.Callback
        public void onLoadCanceled(ParsingLoadable<Long> parsingLoadable, long j, long j6, boolean z4) {
            DashMediaSource.this.onLoadCanceled(parsingLoadable, j, j6);
        }

        @Override // androidx.media3.exoplayer.upstream.Loader.Callback
        public void onLoadCompleted(ParsingLoadable<Long> parsingLoadable, long j, long j6) {
            DashMediaSource.this.onUtcTimestampLoadCompleted(parsingLoadable, j, j6);
        }

        @Override // androidx.media3.exoplayer.upstream.Loader.Callback
        public Loader.LoadErrorAction onLoadError(ParsingLoadable<Long> parsingLoadable, long j, long j6, IOException iOException, int i) {
            return DashMediaSource.this.onUtcTimestampLoadError(parsingLoadable, j, j6, iOException);
        }
    }
    ...
    private static long getAvailableEndTimeInManifestUs(Period period, long j, long j6) {
        ...
        for (int i = 0; i < period.adaptationSets.size(); i++) {
            ...
            if ((!zHasVideoOrAudioAdaptationSets || !z4) && !list.isEmpty()) {
    ...
    private static long getAvailableStartTimeInManifestUs(Period period, long j, long j6) {
        long jMsToUs = Util.msToUs(period.startMs);
        boolean zHasVideoOrAudioAdaptationSets = hasVideoOrAudioAdaptationSets(period);
        long jMax = jMsToUs;
        for (int i = 0; i < period.adaptationSets.size(); i++) {
            ...
            if ((!zHasVideoOrAudioAdaptationSets || !z4) && !list.isEmpty()) {
                ...
                jMax = Math.max(jMax, index.getTimeUs(index.getFirstAvailableSegmentNum(j, j6)) + jMsToUs);
            }
        }
        return jMax;
    }
    ...
    private static long getIntervalUntilNextManifestRefreshMs(DashManifest dashManifest, long j) {
        ...
        for (int i = 0; i < period.adaptationSets.size(); i++) {
            ...
            if (!list.isEmpty() && (index = list.get(0).getIndex()) != null) {
                ...
                if (nextSegmentAvailableTimeUs < jMsToUs4 - SilenceSkippingAudioProcessor.DEFAULT_MINIMUM_SILENCE_DURATION_US || (nextSegmentAvailableTimeUs > jMsToUs4 && nextSegmentAvailableTimeUs < SilenceSkippingAudioProcessor.DEFAULT_MINIMUM_SILENCE_DURATION_US + jMsToUs4)) {
                    jMsToUs4 = nextSegmentAvailableTimeUs;
                }
            }
        }
        return i.k(jMsToUs4, 1000L, RoundingMode.CEILING);
    }
    ...
    private static boolean hasVideoOrAudioAdaptationSets(Period period) {
        for (int i = 0; i < period.adaptationSets.size(); i++) {
            int i4 = period.adaptationSets.get(i).type;
            if (i4 == 1 || i4 == 2) {
                return true;
            }
        }
    ...
    private void processManifest(boolean z4) {
        ...
        if (z5) {
            ...
            if (j9 != C.TIME_UNSET) {
                availableStartTimeInManifestUs = Math.max(availableStartTimeInManifestUs, availableEndTimeInManifestUs - Util.msToUs(j9));
            }
        }
        ...
        if (dashManifest.dynamic) {
            ...
            updateLiveConfiguration(jMsToUs2, j10);
            ...
            j = 0;
            long jMin = Math.min(this.minLiveStartPositionUs, j10 / 2);
            j7 = jMsToUs3 < jMin ? jMin : jMsToUs3;
            j6 = jUsToMs;
        } else {
            j = 0;
        ...
        refreshSourceInfo(new DashTimeline(dashManifest2.availabilityStartTimeMs, j6, this.elapsedRealtimeOffsetMs, this.firstPeriodId, jMsToUs4, j10, j7, dashManifest2, getMediaItem(), this.manifest.dynamic ? this.liveConfiguration : null));
        ...
        if (z4) {
            ...
            if (dashManifest3.dynamic) {
                ...
                if (j11 != C.TIME_UNSET) {
                    ...
                    scheduleManifestRefresh(Math.max(j, (this.manifestLoadStartTimestampMs + j11) - SystemClock.elapsedRealtime()));
                }
            }
        }
    }
    ...
    */
    ...
    public void onLoadCanceled(ParsingLoadable<?> parsingLoadable, long j, long j6) {
        LoadEventInfo loadEventInfo = new LoadEventInfo(parsingLoadable.loadTaskId, parsingLoadable.dataSpec, parsingLoadable.getUri(), parsingLoadable.getResponseHeaders(), j, j6, parsingLoadable.bytesLoaded());
        this.loadErrorHandlingPolicy.onLoadTaskConcluded(parsingLoadable.loadTaskId);
        this.manifestEventDispatcher.loadCanceled(loadEventInfo, parsingLoadable.type);
    }

    public void onManifestLoadCompleted(ParsingLoadable<DashManifest> parsingLoadable, long j, long j6) {
        LoadEventInfo loadEventInfo = new LoadEventInfo(parsingLoadable.loadTaskId, parsingLoadable.dataSpec, parsingLoadable.getUri(), parsingLoadable.getResponseHeaders(), j, j6, parsingLoadable.bytesLoaded());
        this.loadErrorHandlingPolicy.onLoadTaskConcluded(parsingLoadable.loadTaskId);
        ...
        int i = 0;
        while (i < periodCount && this.manifest.getPeriod(i).startMs < j7) {
            i++;
        }
        if (result.dynamic) {
            if (periodCount - i > result.getPeriodCount()) {
                ...
                if (j9 == C.TIME_UNSET || result.publishTimeMs * 1000 > j9) {
            ...
            if (i4 < this.loadErrorHandlingPolicy.getMinimumLoadableRetryCount(parsingLoadable.type)) {
    ...
    public Loader.LoadErrorAction onManifestLoadError(ParsingLoadable<DashManifest> parsingLoadable, long j, long j6, IOException iOException, int i) {
        LoadEventInfo loadEventInfo = new LoadEventInfo(parsingLoadable.loadTaskId, parsingLoadable.dataSpec, parsingLoadable.getUri(), parsingLoadable.getResponseHeaders(), j, j6, parsingLoadable.bytesLoaded());
    ...
    public void onUtcTimestampLoadCompleted(ParsingLoadable<Long> parsingLoadable, long j, long j6) {
        LoadEventInfo loadEventInfo = new LoadEventInfo(parsingLoadable.loadTaskId, parsingLoadable.dataSpec, parsingLoadable.getUri(), parsingLoadable.getResponseHeaders(), j, j6, parsingLoadable.bytesLoaded());
        this.loadErrorHandlingPolicy.onLoadTaskConcluded(parsingLoadable.loadTaskId);
        this.manifestEventDispatcher.loadCompleted(loadEventInfo, parsingLoadable.type);
    ...
    public Loader.LoadErrorAction onUtcTimestampLoadError(ParsingLoadable<Long> parsingLoadable, long j, long j6, IOException iOException) {
        this.manifestEventDispatcher.loadError(new LoadEventInfo(parsingLoadable.loadTaskId, parsingLoadable.dataSpec, parsingLoadable.getUri(), parsingLoadable.getResponseHeaders(), j, j6, parsingLoadable.bytesLoaded()), parsingLoadable.type, iOException, true);
        this.loadErrorHandlingPolicy.onLoadTaskConcluded(parsingLoadable.loadTaskId);
    ...
    private DashMediaSource(MediaItem mediaItem, @Nullable DashManifest dashManifest, @Nullable DataSource.Factory factory, @Nullable ParsingLoadable.Parser<? extends DashManifest> parser, DashChunkSource.Factory factory2, CompositeSequenceableLoaderFactory compositeSequenceableLoaderFactory, @Nullable CmcdConfiguration cmcdConfiguration, DrmSessionManager drmSessionManager, LoadErrorHandlingPolicy loadErrorHandlingPolicy, long j, long j6) {
        this.mediaItem = mediaItem;
        ...
        this.refreshManifestRunnable = new Runnable(this) { // from class: androidx.media3.exoplayer.dash.d
            ...
            public final /* synthetic */ DashMediaSource f7999b;
            ...
            public final void run() {
                switch (i) {
                    case 0:
                        this.f7999b.startLoadingManifest();
                        break;
                    default:
                        this.f7999b.lambda$new$0();
                        break;
                }
            }
        };
