...
final class DefaultMediaClock implements MediaClock {
    ...
    private boolean shouldUseStandaloneClock(boolean z4) {
        Renderer renderer = this.rendererClockSource;
        if (renderer == null || renderer.isEnded()) {
            return true;
        }
        if (z4 && this.rendererClockSource.getState() != 2) {
            return true;
        }
        if (this.rendererClockSource.isReady()) {
            return false;
        }
        return z4 || this.rendererClockSource.hasReadStreamToEnd();
    }

    private void syncClocks(boolean z4) {
        if (shouldUseStandaloneClock(z4)) {
    ...
    public boolean hasSkippedSilenceSinceLastCall() {
        ...
        return B.a(standaloneMediaClock);
    }
