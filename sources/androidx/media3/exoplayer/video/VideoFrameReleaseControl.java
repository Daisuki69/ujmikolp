...
public final class VideoFrameReleaseControl {
    ...
    public interface FrameTimingEvaluator {
        boolean shouldDropFrame(long j, long j6, boolean z4);

        boolean shouldForceReleaseFrame(long j, long j6);

        boolean shouldIgnoreFrame(long j, long j6, long j7, boolean z4, boolean z5) throws ExoPlaybackException;
    }
    ...
    private long calculateEarlyTimeUs(long j, long j6, long j7) {
        long j9 = (long) ((j7 - j) / ((double) this.playbackSpeed));
        return this.started ? j9 - (Util.msToUs(this.clock.elapsedRealtime()) - j6) : j9;
    }
    ...
    private boolean shouldForceRelease(long j, long j6, long j7) {
        ...
        if (i == 3) {
            return this.started && this.frameTimingEvaluator.shouldForceReleaseFrame(j6, Util.msToUs(this.clock.elapsedRealtime()) - this.lastReleaseRealtimeUs);
        }
    ...
    public int getFrameReleaseAction(long j, long j6, long j7, long j9, boolean z4, FrameReleaseInfo frameReleaseInfo) throws ExoPlaybackException {
        ...
        frameReleaseInfo.earlyUs = calculateEarlyTimeUs(j6, j7, j);
        if (shouldForceRelease(j6, frameReleaseInfo.earlyUs, j9)) {
            return 0;
        }
        ...
        boolean z5 = (this.joiningDeadlineMs == C.TIME_UNSET || this.joiningRenderNextFrameImmediately) ? false : true;
        if (this.frameTimingEvaluator.shouldIgnoreFrame(frameReleaseInfo.earlyUs, j6, j7, z4, z5)) {
            return 4;
        }
        return this.frameTimingEvaluator.shouldDropFrame(frameReleaseInfo.earlyUs, j7, z4) ? z5 ? 3 : 2 : frameReleaseInfo.earlyUs > MAX_EARLY_US_THRESHOLD ? 5 : 1;
    }

    public boolean isReady(boolean z4) {
        if (z4 && this.firstFrameState == 3) {
