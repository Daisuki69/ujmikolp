...
public final class DownloadHelper {
    ...
    public static final class MediaPreparer implements MediaSource.MediaSourceCaller, MediaPeriod.Callback, Handler.Callback {
        ...
        private final Handler downloadHelperHandler = Util.createHandlerForCurrentOrMainLooper(new f(this, 0));
    ...
    private void addTrackSelectionInternal(int i, TrackSelectionParameters trackSelectionParameters) throws ExoPlaybackException {
        ...
        runTrackSelection(i);
        P pN = ((S) trackSelectionParameters.overrides.values()).listIterator(0);
        while (pN.hasNext()) {
            this.trackSelector.setParameters(trackSelectionParameters.buildUpon().setOverrideForType((TrackSelectionOverride) pN.next()).build());
            runTrackSelection(i);
        }
    }
    ...
    public void onMediaPreparationFailed(IOException iOException) {
        ((Handler) Assertions.checkNotNull(this.callbackHandler)).post(new j(1, this, iOException));
    }
    ...
    public void onMediaPrepared() throws ExoPlaybackException {
        ...
        for (int i = 0; i < length; i++) {
            for (int i4 = 0; i4 < size; i4++) {
                this.trackSelectionsByPeriodAndRenderer[i][i4] = new ArrayList();
                this.immutableTrackSelectionsByPeriodAndRenderer[i][i4] = Collections.unmodifiableList(this.trackSelectionsByPeriodAndRenderer[i][i4]);
            }
        }
        ...
        for (int i6 = 0; i6 < length; i6++) {
            this.trackGroupArrays[i6] = this.mediaPreparer.mediaPeriods[i6].getTrackGroups();
            this.trackSelector.onSelectionActivated(runTrackSelection(i6).info);
            this.mappedTrackInfos[i6] = (MappingTrackSelector.MappedTrackInfo) Assertions.checkNotNull(this.trackSelector.getCurrentMappedTrackInfo());
        }
        setPreparedWithMedia();
        ((Handler) Assertions.checkNotNull(this.callbackHandler)).post(new k(this, 1));
    }
    ...
    private TrackSelectorResult runTrackSelection(int i) throws ExoPlaybackException {
        ...
        for (int i4 = 0; i4 < trackSelectorResultSelectTracks.length; i4++) {
            ExoTrackSelection exoTrackSelection = trackSelectorResultSelectTracks.selections[i4];
            if (exoTrackSelection != null) {
                List<ExoTrackSelection> list = this.trackSelectionsByPeriodAndRenderer[i][i4];
                int i6 = 0;
                while (true) {
                    if (i6 >= list.size()) {
                    ...
                    if (exoTrackSelection2.getTrackGroup().equals(exoTrackSelection.getTrackGroup())) {
                        ...
                        for (int i10 = 0; i10 < exoTrackSelection2.length(); i10++) {
                            this.scratchSet.put(exoTrackSelection2.getIndexInTrackGroup(i10), 0);
                        }
                        for (int i11 = 0; i11 < exoTrackSelection.length(); i11++) {
                            this.scratchSet.put(exoTrackSelection.getIndexInTrackGroup(i11), 0);
                        }
                        int[] iArr = new int[this.scratchSet.size()];
                        for (int i12 = 0; i12 < this.scratchSet.size(); i12++) {
                            iArr[i12] = this.scratchSet.keyAt(i12);
                        }
    ...
    public void addTextLanguagesToSelection(boolean z4, String... strArr) {
        try {
            assertPreparedWithMedia();
            DefaultTrackSelector.Parameters.Builder builderBuildUpon = DEFAULT_TRACK_SELECTOR_PARAMETERS_WITHOUT_CONTEXT.buildUpon();
            builderBuildUpon.setSelectUndeterminedTextLanguage(z4);
            builderBuildUpon.setForceHighestSupportedBitrate(true);
            for (RendererCapabilities rendererCapabilities : this.rendererCapabilities.getRendererCapabilities()) {
                int trackType = rendererCapabilities.getTrackType();
    ...
    public void addTrackSelectionForSingleRenderer(int i, int i4, DefaultTrackSelector.Parameters parameters, List<DefaultTrackSelector.SelectionOverride> list) {
        try {
            ...
            int i6 = 0;
            while (i6 < this.mappedTrackInfos[i].getRendererCount()) {
                builderBuildUpon.setRendererDisabled(i6, i6 != i4);
                i6++;
            }
            if (list.isEmpty()) {
            ...
            TrackGroupArray trackGroups = this.mappedTrackInfos[i].getTrackGroups(i4);
            for (int i10 = 0; i10 < list.size(); i10++) {
                builderBuildUpon.setSelectionOverride(i4, trackGroups, list.get(i10));
                addTrackSelectionInternal(i, builderBuildUpon.build());
            }
        } catch (ExoPlaybackException e) {
            throw new IllegalStateException(e);
    ...
    public void clearTrackSelections(int i) {
        assertPreparedWithMedia();
        for (int i4 = 0; i4 < this.rendererCapabilities.size(); i4++) {
            this.trackSelectionsByPeriodAndRenderer[i][i4].clear();
        }
    }
    ...
    public List<ExoTrackSelection> getTrackSelections(int i, int i4) {
        assertPreparedWithMedia();
        return this.immutableTrackSelectionsByPeriodAndRenderer[i][i4];
    }
    ...
    public void prepare(Callback callback) {
        ...
        if (mediaSource != null) {
            ...
            this.callbackHandler.post(new j(2, this, callback));
        }
    }
    ...
    public DownloadHelper(MediaItem mediaItem, @Nullable MediaSource mediaSource, TrackSelectionParameters trackSelectionParameters, RendererCapabilitiesList rendererCapabilitiesList) {
        ...
        defaultTrackSelector.init(new e(), new FakeBandwidthMeter());
    ...
    public DownloadRequest getDownloadRequest(String str, @Nullable byte[] bArr) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            for (int i4 = 0; i4 < length2; i4++) {
                arrayList2.addAll(this.trackSelectionsByPeriodAndRenderer[i][i4]);
            }
