...
final class ProgressiveMediaPeriod implements MediaPeriod, ExtractorOutput, Loader.Callback<ExtractingLoadable>, Loader.ReleaseCallback, SampleQueue.UpstreamFormatChangedListener {
    ...
    public final class ExtractingLoadable implements Loader.Loadable, IcyDataSource.Listener {
        ...
        public void setLoadPosition(long j, long j6) {
        ...
        public void load() throws IOException {
            ...
            while (i == 0 && !this.loadCanceled) {
                try {
                    ...
                    this.progressiveMediaExtractor.init(icyDataSource, this.uri, this.dataSource.getResponseHeaders(), j, j6, this.extractorOutput);
    ...
    public interface Listener {
        void onSourceInfoRefreshed(long j, boolean z4, boolean z5);
    }
    ...
    /* JADX INFO: Access modifiers changed from: private */
    public long getLargestQueuedTimestampUs(boolean z4) {
        long jMax = Long.MIN_VALUE;
        for (int i = 0; i < this.sampleQueues.length; i++) {
            if (z4 || ((TrackState) Assertions.checkNotNull(this.trackState)).trackEnabledStates[i]) {
    ...
    public void maybeFinishPrepare() {
        ...
        for (int i = 0; i < length; i++) {
            ...
            boolean z4 = zIsAudio || MimeTypes.isVideo(str);
            zArr[i] = z4;
    ...
    private boolean seekInsideBufferUs(boolean[] zArr, long j) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (!(this.isSingleSample ? sampleQueue.seekTo(sampleQueue.getFirstIndex()) : sampleQueue.seekTo(j, false)) && (zArr[i] || !this.haveAudioVideoTracks)) {
                return false;
            }
        }
        return true;
    ...
    public void lambda$seekMap$1(SeekMap seekMap) {
        ...
        boolean z4 = !this.isLengthKnown && seekMap.getDurationUs() == C.TIME_UNSET;
        this.isLive = z4;
    ...
    public void discardBuffer(long j, boolean z4) {
        ...
        int length = this.sampleQueues.length;
        for (int i = 0; i < length; i++) {
            this.sampleQueues[i].discardTo(j, z4, zArr[i]);
        }
    }
    ...
    public int readData(int i, FormatHolder formatHolder, DecoderInputBuffer decoderInputBuffer, int i4) {
        ...
        int i6 = this.sampleQueues[i].read(formatHolder, decoderInputBuffer, i4, this.loadingFinished);
        if (i6 == -3) {
    ...
    public void seekMap(final SeekMap seekMap) {
        this.handler.post(new Runnable() { // from class: androidx.media3.exoplayer.source.n
            ...
            public final void run() {
                this.f8099a.lambda$seekMap$1(seekMap);
            }
        });
    }
    ...
    public long selectTracks(ExoTrackSelection[] exoTrackSelectionArr, boolean[] zArr, SampleStream[] sampleStreamArr, boolean[] zArr2, long j) {
        ...
        for (int i6 = 0; i6 < exoTrackSelectionArr.length; i6++) {
            ...
            if (sampleStream != null && (exoTrackSelectionArr[i6] == null || !zArr[i6])) {
                int i10 = ((SampleStreamImpl) sampleStream).track;
        ...
        boolean z4 = !this.seenFirstTrackSelection ? j == 0 || this.isSingleSample : i != 0;
        for (int i11 = 0; i11 < exoTrackSelectionArr.length; i11++) {
            if (sampleStreamArr[i11] == null && (exoTrackSelection = exoTrackSelectionArr[i11]) != null) {
                ...
                if (!z4) {
                    ...
                    z4 = (sampleQueue.getReadIndex() == 0 || sampleQueue.seekTo(j, true)) ? false : true;
                }
            }
        }
    ...
    public void onLoadCanceled(ExtractingLoadable extractingLoadable, long j, long j6, boolean z4) {
        StatsDataSource statsDataSource = extractingLoadable.dataSource;
        LoadEventInfo loadEventInfo = new LoadEventInfo(extractingLoadable.loadTaskId, extractingLoadable.dataSpec, statsDataSource.getLastOpenedUri(), statsDataSource.getLastResponseHeaders(), j, j6, statsDataSource.getBytesRead());
    ...
    @Override // androidx.media3.exoplayer.upstream.Loader.Callback
    public void onLoadCompleted(ExtractingLoadable extractingLoadable, long j, long j6) {
        SeekMap seekMap;
        if (this.durationUs == C.TIME_UNSET && (seekMap = this.seekMap) != null) {
            ...
            this.durationUs = j7;
            this.listener.onSourceInfoRefreshed(j7, zIsSeekable, this.isLive);
        }
        StatsDataSource statsDataSource = extractingLoadable.dataSource;
        LoadEventInfo loadEventInfo = new LoadEventInfo(extractingLoadable.loadTaskId, extractingLoadable.dataSpec, statsDataSource.getLastOpenedUri(), statsDataSource.getLastResponseHeaders(), j, j6, statsDataSource.getBytesRead());
        this.loadErrorHandlingPolicy.onLoadTaskConcluded(extractingLoadable.loadTaskId);
    ...
    @Override // androidx.media3.exoplayer.upstream.Loader.Callback
    public Loader.LoadErrorAction onLoadError(ExtractingLoadable extractingLoadable, long j, long j6, IOException iOException, int i) {
