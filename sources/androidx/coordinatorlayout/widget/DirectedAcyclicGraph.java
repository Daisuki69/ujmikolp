...
public final class DirectedAcyclicGraph<T> {
    ...
    public void addEdge(@NonNull T t5, @NonNull T t10) {
        if (!this.mGraph.containsKey(t5) || !this.mGraph.containsKey(t10)) {
            throw new IllegalArgumentException("All nodes must be present in the graph before being added as an edge");
        }
        ArrayList<T> emptyList = this.mGraph.get(t5);
        if (emptyList == null) {
            emptyList = getEmptyList();
            this.mGraph.put(t5, emptyList);
        }
        emptyList.add(t10);
    }
    ...
    public List<T> getOutgoingEdges(@NonNull T t5) {
        ...
        for (int i = 0; i < size; i++) {
            ArrayList<T> arrayListValueAt = this.mGraph.valueAt(i);
            if (arrayListValueAt != null && arrayListValueAt.contains(t5)) {
                if (arrayList == null) {
