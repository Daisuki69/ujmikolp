...
public class Camera2RequestProcessor implements RequestProcessor {
    ...
    public class Camera2CallbackWrapper extends CameraCaptureSession.CaptureCallback {
        ...
        public Camera2CallbackWrapper(@NonNull RequestProcessor.Request request, @NonNull RequestProcessor.Callback callback, boolean z4) {
            this.mCallback = callback;
            this.mRequest = request;
            this.mInvokeSequenceCallback = z4;
        }
        ...
        public void onCaptureStarted(@NonNull CameraCaptureSession cameraCaptureSession, @NonNull CaptureRequest captureRequest, long j, long j6) {
            this.mCallback.onCaptureStarted(this.mRequest, j6, j);
        }
    }
            ...
            try {
                if (!this.mIsClosed && areRequestsValid(list) && this.mCaptureSession != null) {
                    ...
                    for (RequestProcessor.Request request : list) {
                        ...
                        builder.setTemplateType(request.getTemplateId());
                        builder.setImplementationOptions(request.getParameters());
                        builder.addCameraCaptureCallback(CaptureCallbackContainer.create(new Camera2CallbackWrapper(request, callback, z4)));
