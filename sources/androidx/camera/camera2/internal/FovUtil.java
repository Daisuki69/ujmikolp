...
public class FovUtil {
    ...
    public static int focalLengthToViewAngleDegrees(float f, float f3) {
        Preconditions.checkArgument(f > 0.0f, "Focal length should be positive.");
        Preconditions.checkArgument(f3 > 0.0f, "Sensor length should be positive.");
        int degrees = (int) Math.toDegrees(Math.atan(f3 / (f * 2.0f)) * 2.0d);
        Preconditions.checkArgumentInRange(degrees, 0, SessionSplitConfiguration.DEFAULT_MAX_SESSION_DURATION_MINS, "The provided focal length and sensor length result in an invalid view angle degrees.");
        return degrees;
    }
