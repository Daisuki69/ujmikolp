...
class BrowserActionsFallbackMenuAdapter extends BaseAdapter {
    ...
    public View getView(int i, View view, ViewGroup viewGroup) {
        ...
        if (browserActionItem.getIconUri() != null) {
            final y yVarLoadBitmap = BrowserServiceFileProvider.loadBitmap(this.mContext.getContentResolver(), browserActionItem.getIconUri());
            yVarLoadBitmap.addListener(new Runnable() { // from class: androidx.browser.browseractions.BrowserActionsFallbackMenuAdapter.1
                @Override // java.lang.Runnable
                public void run() {
                    Bitmap bitmap;
                    if (TextUtils.equals(title, viewHolderItem.mText.getText())) {
                        try {
                            bitmap = (Bitmap) yVarLoadBitmap.get();
