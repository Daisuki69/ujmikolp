...
public abstract class FragmentStateAdapter extends RecyclerView.Adapter<FragmentViewHolder> implements StatefulAdapter {
    ...
    private Long itemForViewHolder(int i) {
        ...
        for (int i4 = 0; i4 < this.mItemIdToViewHolder.size(); i4++) {
            if (this.mItemIdToViewHolder.valueAt(i4).intValue() == i) {
                if (lValueOf != null) {
                    throw new IllegalStateException("Design assumption violated: a ViewHolder can only be bound to one item at a time.");
                }
                lValueOf = Long.valueOf(this.mItemIdToViewHolder.keyAt(i4));
            }
        }
        return lValueOf;
    }
    ...
    public void gcFragments() {
        ...
        if (!this.mIsInGracePeriod) {
            ...
            for (int i4 = 0; i4 < this.mFragments.size(); i4++) {
                long jKeyAt2 = this.mFragments.keyAt(i4);
        ...
        for (String str : bundle.keySet()) {
            if (isValidKey(str, KEY_PREFIX_FRAGMENT)) {
                ...
                if (!isValidKey(str, KEY_PREFIX_STATE)) {
                    throw new IllegalArgumentException(AbstractC1414e.h("Unexpected key in savedState: ", str));
                }
    ...
    public final Parcelable saveState() {
        ...
        for (int i4 = 0; i4 < this.mSavedStates.size(); i4++) {
            long jKeyAt2 = this.mSavedStates.keyAt(i4);
