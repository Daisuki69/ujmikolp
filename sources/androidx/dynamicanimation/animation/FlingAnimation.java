...
public final class FlingAnimation extends DynamicAnimation<FlingAnimation> {
    ...
    public static final class DragForce implements Force {
        ...
        public DynamicAnimation.MassState updateValueAndVelocity(float f, float f3, long j) {
            float f7 = j;
            this.mMassState.mVelocity = (float) (Math.exp((f7 / 1000.0f) * this.mFriction) * ((double) f3));
            DynamicAnimation.MassState massState = this.mMassState;
            float f10 = this.mFriction;
            massState.mValue = (float) ((Math.exp((f10 * f7) / 1000.0f) * ((double) (f3 / f10))) + ((double) (f - (f3 / f10))));
    ...
    public float getAcceleration(float f, float f3) {
        return this.mFlingForce.getAcceleration(f, f3);
    }
    ...
    @Override // androidx.dynamicanimation.animation.DynamicAnimation
    public boolean isAtEquilibrium(float f, float f3) {
        return f >= this.mMaxValue || f <= this.mMinValue || this.mFlingForce.isAtEquilibrium(f, f3);
    }
