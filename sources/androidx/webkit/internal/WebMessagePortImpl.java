...
public class WebMessagePortImpl extends WebMessagePortCompat {
    ...
    private WebMessagePortBoundaryInterface getBoundaryInterface() {
        if (this.mBoundaryInterface == null) {
            this.mBoundaryInterface = (WebMessagePortBoundaryInterface) jk.b.c(WebMessagePortBoundaryInterface.class, WebViewGlueCommunicator.getCompatConverter().convertWebMessagePort(this.mFrameworksImpl));
        }
    ...
    public void postMessage(WebMessageCompat webMessageCompat) {
        ...
        if (m.isSupportedByFramework() && webMessageCompat.getType() == 0) {
            ...
            getBoundaryInterface().postMessage(new jk.a(new WebMessageAdapter(webMessageCompat)));
        }
    }
    ...
    public void setWebMessageCallback(WebMessagePortCompat.WebMessageCallbackCompat webMessageCallbackCompat) {
        ...
        if (m.isSupportedByWebView()) {
            getBoundaryInterface().setWebMessageCallback(new jk.a(new WebMessageCallbackAdapter(webMessageCallbackCompat)));
        } else {
            if (!m.isSupportedByFramework()) {
                throw WebViewFeatureInternal.getUnsupportedOperationException();
            }
    ...
    public WebMessagePortImpl(InvocationHandler invocationHandler) {
        this.mBoundaryInterface = (WebMessagePortBoundaryInterface) jk.b.c(WebMessagePortBoundaryInterface.class, invocationHandler);
    }
    ...
    public void setWebMessageCallback(Handler handler, WebMessagePortCompat.WebMessageCallbackCompat webMessageCallbackCompat) {
        ...
        if (m.isSupportedByWebView()) {
            getBoundaryInterface().setWebMessageCallback(new jk.a(new WebMessageCallbackAdapter(webMessageCallbackCompat)), handler);
        } else {
            if (m.isSupportedByFramework()) {
