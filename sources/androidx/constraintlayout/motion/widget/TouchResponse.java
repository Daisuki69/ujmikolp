...
class TouchResponse {
    ...
    private void fill(TypedArray typedArray) {
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            if (index == R.styleable.OnSwipe_touchAnchorId) {
                ...
                int i4 = typedArray.getInt(index, this.mTouchAnchorSide);
                this.mTouchAnchorSide = i4;
                float[] fArr = TOUCH_SIDES[i4];
    ...
    public float dot(float f, float f3) {
        return (f3 * this.mTouchDirectionY) + (f * this.mTouchDirectionX);
    }
    ...
    public float getProgressDirection(float f, float f3) {
    ...
    public void processTouchEvent(MotionEvent motionEvent, MotionLayout.MotionTracker motionTracker, int i, MotionScene motionScene) {
        ...
        if (action == 1) {
            ...
            float f7 = !Float.isNaN(fAbs) ? (fAbs / 3.0f) + progress : progress;
            if (f7 == 0.0f || f7 == 1.0f || (i4 = this.mOnTouchUp) == 3) {
                if (0.0f >= f7 || 1.0f <= f7) {
            ...
            float f10 = ((double) f7) < 0.5d ? 0.0f : 1.0f;
            if (i4 == 6) {
        ...
        if (Math.abs((this.mTouchDirectionY * rawY) + (this.mTouchDirectionX * rawX)) > this.mDragThreshold || this.mDragStarted) {
            ...
            if (i10 != -1) {
                ...
                c11 = 0;
                f = progress2;
                this.mMotionLayout.getAnchorDpDt(i10, f, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
            } else {
                c10 = 1;
                f = progress2;
                c11 = 0;
            ...
            if (Math.abs(((this.mTouchDirectionY * fArr4[c10]) + (f11 * fArr4[c11])) * this.mDragScale) < 0.01d) {
                float[] fArr5 = this.mAnchorDpDt;
                fArr5[c11] = 0.01f;
                fArr5[c10] = 0.01f;
            }
            float fMax = Math.max(Math.min(f + (this.mTouchDirectionX != 0.0f ? rawX / this.mAnchorDpDt[c11] : rawY / this.mAnchorDpDt[c10]), 1.0f), 0.0f);
            ...
            if (fMax != progress3) {
                if (progress3 == 0.0f || progress3 == 1.0f) {
                    this.mMotionLayout.endTrigger(progress3 == 0.0f ? c10 : c11);
                }
                this.mMotionLayout.setProgress(fMax);
                motionTracker.computeCurrentVelocity(1000);
                this.mMotionLayout.mLastVelocity = this.mTouchDirectionX != 0.0f ? motionTracker.getXVelocity() / this.mAnchorDpDt[c11] : motionTracker.getYVelocity() / this.mAnchorDpDt[c10];
            } else {
    ...
    */
    ...
    public void scrollMove(float f, float f3) {
        float progress = this.mMotionLayout.getProgress();
        if (!this.mDragStarted) {
            this.mDragStarted = true;
        ...
        float fMax = Math.max(Math.min(progress + (f10 != 0.0f ? (f * f10) / this.mAnchorDpDt[0] : (f3 * this.mTouchDirectionY) / this.mAnchorDpDt[1]), 1.0f), 0.0f);
        if (fMax != this.mMotionLayout.getProgress()) {
            this.mMotionLayout.setProgress(fMax);
        }
    }

    public void scrollUp(float f, float f3) {
        this.mDragStarted = false;
        float progress = this.mMotionLayout.getProgress();
        this.mMotionLayout.getAnchorDpDt(this.mTouchAnchorId, progress, this.mTouchAnchorX, this.mTouchAnchorY, this.mAnchorDpDt);
        float f7 = this.mTouchDirectionX;
        float[] fArr = this.mAnchorDpDt;
        float f10 = f7 != 0.0f ? (f * f7) / fArr[0] : (f3 * this.mTouchDirectionY) / fArr[1];
        if (!Float.isNaN(f10)) {
            progress += f10 / 3.0f;
        }
        if (progress != 0.0f) {
            ...
            if ((i != 3) && z4) {
                this.mMotionLayout.touchAnimateTo(i, ((double) progress) >= 0.5d ? 1.0f : 0.0f, f10);
            }
        }
    }
    ...
    public void setDown(float f, float f3) {
        this.mLastTouchX = f;
        this.mLastTouchY = f3;
    }
    ...
    public void setTouchAnchorLocation(float f, float f3) {
    ...
    public void setUpTouchEvent(float f, float f3) {
        this.mLastTouchX = f;
        this.mLastTouchY = f3;
        this.mDragStarted = false;
    }
    ...
    public void setupTouch() {
        ...
        if (viewFindViewById instanceof NestedScrollView) {
            ...
            nestedScrollView.setOnScrollChangeListener(new NestedScrollView.OnScrollChangeListener(this) { // from class: androidx.constraintlayout.motion.widget.TouchResponse.2
                ...
                public void onScrollChange(NestedScrollView nestedScrollView2, int i4, int i6, int i10, int i11) {
                }
            });
        }
    }
