...
public class StateSet {
    ...
    public static class State {
        ...
        public int findMatch(float f, float f3) {
            for (int i = 0; i < this.mVariants.size(); i++) {
                if (this.mVariants.get(i).match(f, f3)) {
                    return i;
                }
            }
            return -1;
    ...
    public static class Variant {
        ...
        public boolean match(float f, float f3) {
    ...
    */
    ...
    public int convertToConstraintSet(int i, int i4, float f, float f3) {
    ...
    public boolean needsToChange(int i, float f, float f3) {
        int i4 = this.mCurrentStateId;
        if (i4 != i) {
            return true;
        }
        State stateValueAt = i == -1 ? this.mStateList.valueAt(0) : this.mStateList.get(i4);
        int i6 = this.mCurrentConstraintNumber;
        return (i6 == -1 || !stateValueAt.mVariants.get(i6).match(f, f3)) && this.mCurrentConstraintNumber != stateValueAt.findMatch(f, f3);
    }
    ...
    public int stateGetConstraintID(int i, int i4, int i6) {
        return updateConstraints(-1, i, i4, i6);
    }

    public int updateConstraints(int i, int i4, float f, float f3) {
        int iFindMatch;
        if (i == i4) {
            State stateValueAt = i4 == -1 ? this.mStateList.valueAt(0) : this.mStateList.get(this.mCurrentStateId);
            if (stateValueAt == null) {
                return -1;
            }
            return ((this.mCurrentConstraintNumber == -1 || !stateValueAt.mVariants.get(i).match(f, f3)) && i != (iFindMatch = stateValueAt.findMatch(f, f3))) ? iFindMatch == -1 ? stateValueAt.mConstraintID : stateValueAt.mVariants.get(iFindMatch).mConstraintID : i;
        }
        State state = this.mStateList.get(i4);
        if (state == null) {
            return -1;
        }
        int iFindMatch2 = state.findMatch(f, f3);
