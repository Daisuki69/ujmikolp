...
public class PriorityGoalRow extends ArrayRow {
    ...
    public class GoalVariableAccessor {
        ...
        public boolean addToGoal(SolverVariable solverVariable, float f) {
            ...
            if (!this.variable.inGoal) {
                for (int i = 0; i < 9; i++) {
                    ...
                    if (f3 != 0.0f) {
                        ...
                        this.variable.goalStrengthVector[i] = f7;
                    } else {
                        this.variable.goalStrengthVector[i] = 0.0f;
                    }
                }
            ...
            for (int i4 = 0; i4 < 9; i4++) {
                float[] fArr = this.variable.goalStrengthVector;
                float f10 = (solverVariable.goalStrengthVector[i4] * f) + fArr[i4];
                fArr[i4] = f10;
        ...
        public String toString() {
            ...
            if (this.variable != null) {
                for (int i = 0; i < 9; i++) {
                    strN = s.n(a.w(strN), Global.BLANK, this.variable.goalStrengthVector[i]);
                }
            }
            StringBuilder sbV = s.v(strN, "] ");
            sbV.append(this.variable);
            return sbV.toString();
        }
    }
    ...
    private final void addToGoal(SolverVariable solverVariable) {
        ...
        int i10 = i6 + 1;
        this.numGoals = i10;
        if (i10 > 1 && solverVariableArr3[i6].id > solverVariable.id) {
            int i11 = 0;
            while (true) {
                i = this.numGoals;
                if (i11 >= i) {
    ...
    */
    ...
    public String toString() {
        String strN = s.n(new StringBuilder(" goal -> ("), ") : ", this.constantValue);
        for (int i = 0; i < this.numGoals; i++) {
            this.accessor.init(this.arrayGoals[i]);
            StringBuilder sbW = a.w(strN);
            sbW.append(this.accessor);
            sbW.append(Global.BLANK);
            strN = sbW.toString();
        }
        return strN;
    }
