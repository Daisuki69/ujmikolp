...
public final class UnknownFieldSetLite {
    ...
    public int getSerializedSize() {
        ...
        for (int i4 = 0; i4 < this.count; i4++) {
            ...
            if (tagWireType == 0) {
                ...
                serializedSize = ((UnknownFieldSetLite) this.objects[i4]).getSerializedSize() + (CodedOutputStream.computeTagSize(tagFieldNumber) * 2) + serializedSize;
            } else {
                if (tagWireType != 5) {
    ...
    public int getSerializedSizeAsMessageSet() {
        ...
        for (int i4 = 0; i4 < this.count; i4++) {
            iComputeRawMessageSetExtensionSize += CodedOutputStream.computeRawMessageSetExtensionSize(WireFormat.getTagFieldNumber(this.tags[i4]), (ByteString) this.objects[i4]);
        }
    ...
    public UnknownFieldSetLite mergeVarintField(int i, int i4) {
        checkMutable();
        if (i == 0) {
            throw new IllegalArgumentException("Zero is not a valid field number.");
        }
        storeField(WireFormat.makeTag(i, 0), Long.valueOf(i4));
        return this;
    }

    public final void printWithIndent(StringBuilder sb2, int i) {
        for (int i4 = 0; i4 < this.count; i4++) {
            MessageLiteToString.printField(sb2, i, String.valueOf(WireFormat.getTagFieldNumber(this.tags[i4])), this.objects[i4]);
        }
    }
    ...
    private UnknownFieldSetLite(int i, int[] iArr, Object[] objArr, boolean z4) {
