...
public class MapEntryLite<K, V> {
    ...
    private MapEntryLite(WireFormat.FieldType fieldType, K k8, WireFormat.FieldType fieldType2, V v7) {
        this.metadata = new Metadata<>(fieldType, k8, fieldType2, v7);
        this.key = k8;
        this.value = v7;
    }

    public static <K, V> int computeSerializedSize(Metadata<K, V> metadata, K k8, V v7) {
        return FieldSet.computeElementSize(metadata.keyType, 1, k8) + FieldSet.computeElementSize(metadata.valueType, 2, v7);
    }

    public static <K, V> MapEntryLite<K, V> newDefaultInstance(WireFormat.FieldType fieldType, K k8, WireFormat.FieldType fieldType2, V v7) {
        return new MapEntryLite<>(fieldType, k8, fieldType2, v7);
    }

    /* JADX WARN: Multi-variable type inference failed */
    public static <T> T parseField(CodedInputStream codedInputStream, ExtensionRegistryLite extensionRegistryLite, WireFormat.FieldType fieldType, T t5) throws IOException {
    ...
    public static <K, V> void writeTo(CodedOutputStream codedOutputStream, Metadata<K, V> metadata, K k8, V v7) throws IOException {
    ...
    public int computeMessageSize(int i, K k8, V v7) {
        return CodedOutputStream.computeLengthDelimitedFieldSize(computeSerializedSize(this.metadata, k8, v7)) + CodedOutputStream.computeTagSize(i);
    }
    ...
    public void serializeTo(CodedOutputStream codedOutputStream, int i, K k8, V v7) throws IOException {
        codedOutputStream.writeTag(i, 2);
        codedOutputStream.writeUInt32NoTag(computeSerializedSize(this.metadata, k8, v7));
        writeTo(codedOutputStream, this.metadata, k8, v7);
    }
    ...
    private MapEntryLite(Metadata<K, V> metadata, K k8, V v7) {
