...
public final class Response<T> {
    ...
    public static <T> Response<T> error(int i, ResponseBody responseBody) {
        ...
        throw new IllegalArgumentException(s.f(i, "code < 400: "));
    }
    ...
    public static <T> Response<T> success(int i, T t5) {
        if (i >= 200 && i < 300) {
            return success(t5, new Response.Builder().code(i).message("Response.success()").protocol(Protocol.HTTP_1_1).request(new Request.Builder().url("http://localhost/").build()).build());
        }
        throw new IllegalArgumentException(s.f(i, "code < 200 or >= 300: "));
    }
