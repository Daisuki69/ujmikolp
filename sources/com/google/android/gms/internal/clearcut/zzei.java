...
class zzei<K extends Comparable<K>, V> extends AbstractMap<K, V> {
    ...
    */
    ...
    public final V zzal(int i) {
        zzdu();
        V v7 = (V) this.zzom.remove(i).getValue();
    ...
    public final V put(K k8, V v7) {
        ...
        int iZza = zza(k8);
        if (iZza >= 0) {
            return (V) this.zzom.get(iZza).setValue(v7);
        }
        zzdu();
        ...
        int i = -(iZza + 1);
        if (i >= this.zzol) {
            return zzdv().put(k8, v7);
        }
        int size = this.zzom.size();
        int i4 = this.zzol;
        if (size == i4) {
            zzep zzepVarRemove = this.zzom.remove(i4 - 1);
            zzdv().put((Comparable) zzepVarRemove.getKey(), zzepVarRemove.getValue());
        }
        this.zzom.add(i, new zzep(this, k8, v7));
        return null;
    }
}
