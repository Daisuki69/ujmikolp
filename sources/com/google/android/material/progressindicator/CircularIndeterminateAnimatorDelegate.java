...
final class CircularIndeterminateAnimatorDelegate extends IndeterminateAnimatorDelegate<ObjectAnimator> {
    ...
    private void maybeUpdateSegmentColors(int i) {
        for (int i4 = 0; i4 < 4; i4++) {
            float fractionInRange = getFractionInRange(i, DELAY_TO_FADE_IN_MS[i4], 333);
            if (fractionInRange >= 0.0f && fractionInRange <= 1.0f) {
                int i6 = i4 + this.indicatorColorIndexOffset;
    ...
    private void updateSegmentPositions(int i) {
        ...
        fArr[1] = f * 1520.0f;
        for (int i4 = 0; i4 < 4; i4++) {
            float fractionInRange = getFractionInRange(i, DELAY_TO_EXPAND_IN_MS[i4], 667);
            float[] fArr2 = this.segmentPositions;
            fArr2[1] = (this.interpolator.getInterpolation(fractionInRange) * 250.0f) + fArr2[1];
            float fractionInRange2 = getFractionInRange(i, DELAY_TO_COLLAPSE_IN_MS[i4], 667);
        ...
        float f3 = fArr4[0];
        float f7 = fArr4[1];
        float f10 = ((f7 - f3) * this.completeEndFraction) + f3;
        fArr4[0] = f10;
        fArr4[0] = f10 / 360.0f;
        fArr4[1] = f7 / 360.0f;
    }
