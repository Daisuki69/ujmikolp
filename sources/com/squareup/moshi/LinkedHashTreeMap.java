...
final class LinkedHashTreeMap<K, V> extends AbstractMap<K, V> implements Serializable {
    ...
    public Node<K, V> find(K k8, boolean z4) {
        int iCompareTo;
        Node<K, V> node;
        Comparator<? super K> comparator = this.comparator;
        Node<K, V>[] nodeArr = this.table;
        int iSecondaryHash = secondaryHash(k8.hashCode());
        ...
        if (node2 != null) {
            ...
            while (true) {
                iCompareTo = comparable != null ? comparable.compareTo(node2.key) : comparator.compare(k8, node2.key);
                if (iCompareTo == 0) {
                    return node2;
                }
        ...
        if (node2 != null) {
            ...
            node = new Node<>(node5, k8, iSecondaryHash, node4, node4.prev);
            if (i < 0) {
                node5.left = node;
            } else {
                node5.right = node;
            ...
            node = new Node<>(node2, k8, iSecondaryHash, node4, node4.prev);
            nodeArr[length] = node;
        }
        int i4 = this.size;
        this.size = i4 + 1;
    ...
    public V put(K k8, V v7) {
        ...
        Node<K, V> nodeFind = find(k8, true);
        V v8 = nodeFind.value;
        nodeFind.value = v7;
        return v8;
    }
    ...
    public void removeInternal(Node<K, V> node, boolean z4) {
        int i;
        if (z4) {
        ...
        nodeLast.height = Math.max(i, i4) + 1;
    ...
    public static <K, V> Node<K, V>[] doubleCapacity(Node<K, V>[] nodeArr) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (node != null) {
                ...
                nodeArr2[i] = i4 > 0 ? avlBuilder.root() : null;
                nodeArr2[i + length] = i6 > 0 ? avlBuilder2.root() : null;
            }
        }
        return nodeArr2;
    }
    ...
    public static final class Node<K, V> implements Map.Entry<K, V> {
        ...
        public Node(Node<K, V> node, K k8, int i, Node<K, V> node2, Node<K, V> node3) {
            this.parent = node;
            this.key = k8;
            this.hash = i;
            this.height = 1;
